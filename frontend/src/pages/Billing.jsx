...
const Billing = () => {
  ...
  const plans = [
    ...
    },
    {
      ...
      color: 'border-indigo-500/30 bg-indigo-950/5 hover:border-indigo-500/60 shadow-lg shadow-indigo-500/5'
    },
    {
      name: 'Silver Plan',
      price: '₹300',
      ...
      color: 'border-purple-500/30 bg-purple-950/5 hover:border-purple-500/60 shadow-lg shadow-purple-500/5'
    },
    {
      name: 'Gold Plan',
      price: '₹1000',
      ...
      color: 'border-amber-500/30 bg-amber-950/5 hover:border-amber-500/60 shadow-lg shadow-amber-500/5'
    }
  ];
  ...
  return (
    <div className="space-y-8 page-transition" id="billing">
      ...
      {/* Current plan banner */}
      <div className="glass-premium rounded-3xl p-6 border border-indigo-500/15 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400">
            <Award size={24} />
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">{t('current_plan')}</p>
            <h3 className="text-xl font-bold text-slate-100 mt-0.5">{user?.activePlan?.planName || 'Free Plan'}</h3>
        ...
        <div className="text-left md:text-right">
          <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Account Premium Status</p>
      ...
      {!isPaymentHour && (
        <div className="bg-rose-500/10 border border-rose-500/20 p-5 rounded-3xl flex items-start gap-3.5 text-rose-350">
          ...
          <div className="text-xs space-y-1">
            ...
            <p className="text-rose-400/90 font-mono mt-1 text-[11px] flex items-center gap-1">
              <Clock size={12} />
              Current IST Simulated Time Hour: {istTime.hours} (Allowed hours: 10 AM - 11 AM IST). Change Simulated Hour inside our rules simulator to purchase premium plans.
            </p>
          </div>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-bold Outfit text-slate-200">{p.name}</h4>
                  <div className="flex items-baseline gap-1 mt-2">
                    <span className="text-3xl font-extrabold Outfit text-slate-100">{p.price}</span>
                    <span className="text-xs text-slate-500">/ {p.duration}</span>
                  </div>
                  <p className="text-[11px] text-indigo-400 font-semibold mt-2.5 bg-indigo-950/20 px-2 py-0.5 rounded border border-indigo-900/30 inline-block">
                    {p.applications}
                  </p>
                </div>
              ...
              <div className="mt-8 pt-4">
                {isCurrent ? (
                  <div className="w-full text-center bg-indigo-600/10 text-indigo-400 border border-indigo-500/20 py-3 rounded-xl text-xs font-bold uppercase tracking-wider">
                    Your Active Plan
                  </div>
                ) : isFree ? (
                  <div className="w-full text-center bg-slate-800 text-slate-500 py-3 rounded-xl text-xs font-semibold uppercase tracking-wider">
                    Included by Default
                  </div>
                ) : (
                  <button
      ...
      <div className="glass-card rounded-3xl p-6">
        <h3 className="text-base font-bold Outfit text-indigo-300 uppercase tracking-wider mb-4 flex items-center gap-2">
          <FileText size={18} />
          {t('login_history')} & Transaction Receipts
        </h3>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-850">
                {invoiceHistory.map((inv) => (
                  <tr key={inv._id} className="hover:bg-slate-800/10 transition-colors text-slate-300">
                    <td className="py-3.5 px-4 font-bold text-slate-200">{inv.invoiceNumber}</td>
                    <td className="py-3.5 px-4 font-medium text-slate-200">{inv.planName}</td>
                    <td className="py-3.5 px-4 font-mono text-[10px] text-indigo-300">{inv.paymentId}</td>
